...
codex-hud 在 LazyCat-Scripts 中独立发布，使用标签 `codex-hud-vX.Y.Z`。用户只下载程序，不需要克隆源码或安装语言运行时。首个稳定版为 [0.1.0](https://github.com/KroMiose/LazyCat-Scripts/releases/tag/codex-hud-v0.1.0)，默认安装版本由 `stable.txt` 指定。
...
- 官方支持独立程序和安装器分发。包管理器集成需将配置卸载与程序删除分开，由包管理器管理可执行文件。
...
`stable.txt` 只指向完成公开安装验证的版本。`unreleased` 是没有稳定版时的保留值，不用于已发布稳定版。如果草稿阶段失败，修正后重跑允许更新草稿；正式发布后禁止覆盖资产。公开安装验证失败时不提升 stable，先检查分发问题，必要时发布新版本；仅重跑失败的后续作业。

PR 检查使用事件记录中的 head/base 提交，在本地重建合并结果；不依赖 PR 关闭后可能消失的临时 merge ref。只有检查通过后才合并 PR。修复工作流后，可用 workflow_dispatch 的 `release_tag` 输入恢复未公开版本的发布，所有构建仍检出原标签。
