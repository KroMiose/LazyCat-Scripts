...
def main():
    ...
    complete=not any(inventory_delta.values())
    if not complete:
        print('Reviewed Python case inventory failed: '+json.dumps(inventory_delta),file=sys.stderr)
    ...
    if not complete:
        ...
        ET.SubElement(case,'failure').text=json.dumps(inventory_delta)
    ...
    summary+='\nReviewed case inventory: '+('matched' if complete else '**FAILED** '+json.dumps(inventory_delta))+'.\n'
