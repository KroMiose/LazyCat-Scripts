...
The additional default-CA fixtures come from the original c8ffb57 client and common library. With a real CA key present, the original command must fail on the literal tilde path while preserving its existing certificate; the repaired command then proves successful signing and login. Fixtures test specific regressions and are not a complete packaged historical upgrade.
