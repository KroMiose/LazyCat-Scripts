...
class RequiredChecks(unittest.TestCase):
    def test_success_and_known_failure(self):
        env=dict(MAPPING='success',QUALITY='success',EXPECT_BEHAVIOR='true',EXPECT_SYSTEM='true',
        ...
        for result in ('skipped','cancelled','failure',''):
            self.assertTrue(failures({**env,'SYSTEM':result}))
        for missing in ('EXPECT_BEHAVIOR','EXPECT_SYSTEM','QUALITY','MAPPING','EVENT'):
