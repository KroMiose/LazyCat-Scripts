...
旧Shell与Go渲染均拒绝生成别名冲突，例如 `box` 的 `box-lan` 线路别名与另一个名为 `box-lan` 的清单主机重叠。旧Shell会在写配置前停止，保留已生效配置；有此历史冲突的清单需改名后再同步，避免依赖Host顺序选择错误目标。

旧Shell同步会在写入任何SSH配置前检查主配置的托管标记以及两个目标的普通文件类型。损坏标记、目录或FIFO目标会停止处理，保留当前正文和权限；需要修复冲突后再同步。该预检不替代尚未完成的双文件中断事务。
