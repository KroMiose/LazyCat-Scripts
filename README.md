...
这是一个极致简化的交互式配置向导，旨在为您快速搭建一个基于 `pipx` 的现代化 Python **工具**环境。它不再管理 Python 版本本身，而是利用您系统已有的 `python3`，并默认安装 `poetry` 和 `pdm`。

- **✨ 主要功能:**

  - **目标专一:** 脚本的核心目标是安装 `pipx`，并用它来管理您的 Python 应用，保持全局环境干净。
  - **默认安装工具:** 脚本会自动为您安装 `poetry` 和 `pdm`，无需任何额外交互。
  - **强大的网络配置:**
    - 内置交互式的网络代理向导，确保在受限网络环境下也能成功下载。
    - 支持一键切换到国内 PyPI 镜像源，极大提升依赖下载速度。
  - **官方安装方式:** 采用 `pipx` 官方推荐的引导脚本进行安装，以获得最佳的系统兼容性和稳定性，完美避开 `externally-managed-environment` 等系统级问题。
...
- **💣 执行副作用:**
  ...
  - **系统级变更:**
    - 脚本会请求 `sudo` 权限来安装 `python3-pip`, `python3-venv`, `git`, `curl` 等基础依赖包。
  - **用户级变更:**
    - 在 `~/.local/` 目录下安装 `pipx` 及其管理的工具 (`poetry`, `pdm` 等)。
    - `pipx ensurepath` 命令会自动尝试向您的 shell 配置文件 (如 `.bashrc`, `.zshrc`, `.profile`) 中添加路径。

- **🔁 可重复执行性:**

  - 本脚本是**完全可重复执行的**。它会智能地跳过已完成的步骤。例如，如果您已经安装了 `pipx`，再次运行脚本时会直接进入工具安装环节。

- **🚀 一键执行:**

  > 您需要使用 `sudo` 来运行此脚本，因为它需要先确保系统基础依赖已安装。脚本后续会智能地切换到您的普通用户身份来完成所有用户级别的配置。
