...
这是一个极致简化的交互式配置向导，旨在为您快速搭建一个现代化的 Python **工具**环境。它不再管理 Python 版本，而是利用您系统已有的 `python3`，并通过最直接的方式为您安装 `poetry` 和 `pdm`。

- **✨ 主要功能:**

  - **目标专一:** 脚本的核心目标是为您准备好 `poetry` 和 `pdm` 这两个现代化的 Python 项目管理工具。
  - **混合安装策略:**
    - **Poetry:** 直接通过系统的 `apt` 包管理器安装 (`python3-poetry`)，确保最佳的系统集成度和稳定性。
    - **PDM:** 由于 `pdm` 未被收录在官方源，脚本会采用其官方推荐的安装脚本进行安装，确保可靠性。
  - **强大的网络配置:**
...
- **💣 执行副作用:**
  ...
  - **系统级变更:**
    - 脚本会请求 `sudo` 权限来安装 `python3-poetry` 及 `python3-pip` 等基础依赖包。
  - **用户级变更:**
    - `pdm` 会被安装到您的用户目录 (`~/.pdm`) 下。
    - `pdm` 的安装脚本会自动尝试向您的 shell 配置文件 (如 `.bashrc`, `.zshrc`) 中添加路径。

- **🔁 可重复执行性:**

  - 本脚本是**完全可重复执行的**。它会智能地跳过已完成的步骤。

- **🚀 一键执行:**

  > 您需要使用 `sudo` 来运行此脚本，因为它需要先确保系统基础依赖已安装。脚本后续会智能地切换到您的普通用户身份来完成 `pdm` 的用户级别配置。
