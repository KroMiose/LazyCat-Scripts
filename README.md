...
这是一个强大的交互式配置向导，旨在为 Linux 用户一键构建一个干净、现代且多版本共存的 Python 开发环境。它使用 `pyenv` 来管理 Python 版本，并使用 `pipx` 来安装和管理命令行工具，确保全局环境的纯净。

- **✨ 主要功能:**

  - **全自动依赖处理:** 自动检测您的 Linux 发行版 (Debian/Ubuntu, RHEL/Fedora/CentOS, Arch) 并安装 `pyenv` 所需的所有编译依赖。
  - **安装 `pyenv`:** 自动从 GitHub 安装 `pyenv`，并将其初始化配置无缝添加到您的 `.bashrc` 和 `.zshrc` 中。
  - **交互式 Python 版本选择:** 提供一个清晰的菜单，让您可以从一系列稳定版本 (3.8 - 3.12) 中选择您想要的 Python 版本进行安装，并自动设为全局默认。
  - **现代化工具链:**
    - 自动安装 `pipx`，这是目前隔离和运行 Python CLI 应用的最佳实践工具。
    - 交互式地询问您是否需要安装 `poetry` 和 `pdm` 这两个流行的依赖管理和打包工具。
  - **清晰的用户指引:** 脚本执行的每一步都有清晰的日志输出，并在结束后给出明确的指引，告诉您需要重启终端才能使一切生效。

- **💻 支持系统:**

  - 基于 Debian/Ubuntu 的发行版。
  - 基于 RHEL/CentOS/Fedora 的发行版。
  - Arch Linux。

- **💣 执行副作用:**

  - **系统级变更:** 脚本会请求 `sudo` 权限来安装 `pyenv` 所需的编译依赖包。
  - **文件与目录操作:**
    - 在 `~/.pyenv` 目录下克隆并安装 `pyenv`。
    - `pipx` 及其安装的工具默认位于 `~/.local` 目录下。
  - **修改用户配置:**
    - 脚本会向 `~/.bashrc` 和 `~/.zshrc` 文件中追加 `pyenv` 的初始化配置。
    - `pipx ensurepath` 命令同样会修改您的 shell 配置文件以更新 `PATH` 变量。

- **🔁 可重复执行性:**

  - 本脚本是**完全可重复执行的**。它会检查 `pyenv` 是否已安装，检查 Python 版本是否已存在，并检查 shell 配置是否已写入，对已完成的步骤会智能跳过，不会造成任何重复配置或破坏。

- **🚀 一键执行:**

  > 您需要使用 `sudo` 来运行此脚本，因为它需要安装系统级的编译依赖。脚本本身会智能地使用 `sudo -u` 来为您的普通用户完成所有配置。
