...
首次完整工作流 run 34160052011（449029d）最终通过：三套系统各两个独立生命周期、Ubuntu/Debian 无缓存镜像/包重建、真实上游 Node/Python 安装与解释器行为、Ubuntu 实际 Docker 上游安装，以及同时执行的四平台候选/原生任务检查。它早于 OpenWrt 固定源与后续修复，只证明当时已登记的范围；并非最终计划全覆盖。
