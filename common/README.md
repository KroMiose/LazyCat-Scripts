...
Zsh 的 `.zshrc` 修改使用统一文件事务，新备份与阶段记录位于 `.zshrc.lazycat-operation.*`，支持检查器的 `recover-lock` 和 `rollback`。旧 `.lazycat.bak.*` 与旧暂存目录不清理；旧 `.lazycat-zsh.lock` 存在时先停止并要求审阅，不擅自判定它已失效。GNU/Linux 更新和恢复显式保留扩展属性。此回滚只处理 `.zshrc`，不卸载已安装的依赖、OMZ、主题或插件。

Host 片段在多文件提交失败时，恢复前复查发布版本，并保留原文件属性。后来修改过的片段不覆盖，相关记录保留为待处理；不能把部分文件已恢复视为整个操作成功。
